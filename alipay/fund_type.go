...
type FundAuthOrderAppFreeze struct {
	...
	DepositProductMode string                              `json:"deposit_product_mode,omitempty"` //免押受理台模式【必选条件】使用免押产品必传该字段 后付金额已知: POSTPAY, 后付金额未知: POSTPAY_UNCERTAIN, 纯免押: DEPOSIT_ONLY
	PostPayments       []FundAuthOrderAppFreezePostPayment `json:"post_payments,omitempty"`
}

type FundAuthOrderAppFreezePostPayment struct {
	Name        string `json:"name"`
	Amount      string `json:"amount,omitempty"`
	Description string `json:"description"`
}

type FundAuthOrderAppFreezeExtraParam struct {
...
func (f FundAuthOrderAppFreeze) NeedEncrypt() bool {
	return false
}
